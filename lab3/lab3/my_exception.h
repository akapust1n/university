...
public:
    file_read_error() {}
    virtual const char* what() const throw() { return "file read  error \n"; }
};
// Класс ошибки памяти
class memory_alloc_error : public my_base_exception {
public:
    memory_alloc_error() {}
    virtual const char* what() const throw() { return "memory alloc error \n"; }
};

//это должно работать, но почему-то мультиопределение
