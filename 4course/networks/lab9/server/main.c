    ...
    do {
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            ...
            if (clientsFD[i].m_context) {
                clock_t difference = clock() - clientsFD[i].m_context->lastModification;
                double diff = difference * 1.0 / CLOCKS_PER_SEC;
                printf("%f\n diff", diff);
                if (diff > TIMEOUT) {
    ...
    while (1) {
        ...
        for (int i = 0; i < numFD; i++) {
            if (events[i].data.fd == listener) {
                if (newClient(listener, epollFD, clientsFD, &numClients, bd) < 0) {
                    perror("Can create new client!");
                }
            } else {
                if (handleMessage(clientsFD, events[i], &numClients) < 0) {
                    perror("Cant handle message!");
                }
            }
        }
    }
