...
const int N=300; //количество элементов массив
struct Queue1
{
...
int main() {
 ...
 cout<<Size(&Q);
        ...
        if(Time == TimeEnter) {
          ...
          if (real)  Add(&Q,FTypeReq); else add(&Q1,FTypeReq);
            ...
            {NextEvent = Clear;
             ...
             else { if (Size(&Q)==1) WasteTime+=Time;} }
          if (real ) if(secType == Top(&Q)) {
    ...
    cout <<"Show adresses of last added/deleted 50 elements(Y/N)? ";
    char b1;
    cin>>b1;
    if (b1=='Y')
