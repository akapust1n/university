...
printf("stroka%s\n",stroka);
if(isMinusPlus(f[i]))
  if (p>0&&isOperator(f[i-1])) add_stroka(stroka,'0');
...
for(int i=0;i<MAX_SIZE;i++)
    yy[i].variable='\0';
printf("len %d", strlen(stroka));
for(int i=0;i<strlen(stroka);i++)
{
    //int u=stroka[i];
    //добавляем
    printf("u_%c",stroka[i]);
    if (isCharacter(stroka[i])&&notbeEarly(stroka[i],yy)) yy[j++].variable=stroka[i];
}
printf("\nEnter variables\n");
for(int i=0;i<9;i++) printf("%c", yy[i].variable);
for(int i=0;yy[i].variable;i++)
{
...
for(int i=0;i<strlen(stroka);i++)
{
 ...
 if (isCharacter(stroka[i])) {add_array_int(fromChartoNum(stroka[i],yy));
     printf("values:%c %d\n",stroka[i],fromChartoNum(stroka[i],yy));printf("%d\n",top_array_int()) ;}
...
//http://www.abakbot.ru/net/181-obratnaya-polskaya-notatsiya-konvertatsiya-onlajn
